...
namespace SiproDAO.Dao
{
    public class EntidadDAO
    {
        ...
        public static List<Entidad> getEntidadesPagina(int pagina, int registros, String filtro_entidad, String filtro_nombre, String filtro_abreviatura,
            ...
            try
            {
                using (DbConnection db = new OracleContext().getConnection())
                {
                    ...
                    if (filtro_entidad != null && filtro_entidad.Trim().Length > 0)
                        query_a = String.Join("", query_a, " TO_CHAR(e.entidad) LIKE :filtro_entidad ");
                    ...
                    ret = db.Query<Entidad>(query, new { filtro_entidad = filtro_entidad, filtro_abreviatura = filtro_abreviatura }).AsList<Entidad>();
                }
            }
            catch (Exception e)
            {
        ...
        public static long getTotalEntidades(String filtro_entidad, String filtro_nombre, String filtro_abreviatura)
        {
            ...
            try
            {
                using (DbConnection db = new OracleContext().getConnection())
                {
                    ...
                    ret = db.ExecuteScalar<long>(query, new { filtro_entidad = filtro_entidad, filtro_abreviatura = filtro_abreviatura });
                }
            }
            catch (Exception e)
            {
