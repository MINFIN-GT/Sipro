...
namespace SProducto.Controllers
{
	...
	public class ProductoValidator : AbstractValidator<JObject>
	{
		public ProductoValidator() 
		{
			...
			RuleFor(producto => producto["nombre"].ToString()).Cascade(CascadeMode.StopOnFirstFailure).NotEmpty().WithMessage("nombre - vacío").Must(producto => { return GenericValidatorType.ValidateType(producto, typeof(String)); }).WithMessage("Error when trying to parse the property 'nombre' to type " + typeof(String)).MaximumLength(1000);
			...
			RuleFor(producto => producto["productoTipoid"].ToString()).Cascade(CascadeMode.StopOnFirstFailure).NotEmpty().WithMessage("productoTipoid - vacío").Must(producto => { return GenericValidatorType.ValidateType(producto, typeof(Int32)); }).WithMessage("Error when trying to parse the property 'productoTipoid' to type " + typeof(Int32));
			...
			RuleFor(producto => producto["ueunidadEjecutora"].ToString()).Cascade(CascadeMode.StopOnFirstFailure).NotEmpty().WithMessage("ueunidadEjecutora - vacío").Must(producto => { return GenericValidatorType.ValidateType(producto, typeof(Int32)); }).WithMessage("Error when trying to parse the property 'ueunidadEjecutora' to type " + typeof(Int32));
