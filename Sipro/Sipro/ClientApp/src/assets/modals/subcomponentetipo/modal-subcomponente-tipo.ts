...
export class DialogOverviewSubComponenteTipo {
  constructor(public dialog: MatDialog) {}
}

@Component({
  selector: 'modal-subcomponente-tipo.ts',
  templateUrl: './modal-dialog.html'
})
export class DialogSubComponenteTipo {
  ...
  constructor(public dialog: MatDialog,
    public dialogRef: MatDialogRef<DialogSubComponenteTipo>,
  ...
  obtenerTotalCodigos(){
    ...
    this.http.post('http://localhost:60082/api/SubcomponenteTipo/NumeroSubComponenteTipos',data,{withCredentials: true}).subscribe(response => {
      if (response['success'] == true) {   
        this.totalElementos = response["totalsubcomponentetipos"];
        this.paginaActual = 1;
