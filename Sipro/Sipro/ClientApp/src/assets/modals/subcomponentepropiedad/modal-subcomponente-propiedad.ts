...
export class DialogOverviewSubComponentePropiedad {
  constructor(public dialog: MatDialog) {}
}

@Component({
  selector: 'modal-subcomponente-propiedad.ts',
  templateUrl: './modal-dialog.html'
})
export class DialogSubComponentePropiedad {
  ...
  constructor(public dialog: MatDialog,
    public dialogRef: MatDialogRef<DialogSubComponentePropiedad>,
  ...
  ngOnInit() { 
    this.obtenerTotalSubComponentesPropiedades();
  }
  ...
  obtenerTotalSubComponentesPropiedades(){
    this.esColapsado = false;
    var filtro = {
      filtro_busqueda: this.busquedaGlobal
    };
    this.http.post('http://localhost:60081/api/SubcomponentePropiedad/NumeroSubComponentePropiedades',filtro, {withCredentials: true}).subscribe(response => {
      if (response['success'] == true) {   
        this.totalElementos = response["totalsubcomponentepropiedades"];
        this.paginaActual = 1;
